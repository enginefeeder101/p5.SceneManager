...
( function ( window )
{
    const p5SceneManager = function ( parent )
    {
        ...
        this.wire = function ( extraEvents )
        {
            ...
            const me = this;
            this.parentObj = parent != null ? parent : window;

            // Wire draw manually for speed reasons...
            this.parentObj.draw = function ()
            {
                me.draw();
            };
            ...
            for ( let i = 0; i < P5Events.length; i++ )
            {
                ...
                const parentEvent = this.parentObj[ sEvent ]; // Preserve parent / global function
                this.parentObj[ sEvent ] = function ()
                {
                    if ( parentEvent ) // If global / parent function exists, call first
                        parentEvent.apply( this.parentObj, [].slice.call( arguments ) );
        ...
        {
            const oScene = new fnScene( this.parentObj );

            // inject parent as a property of the scene
            oScene.parent = this.parentObj;
        ...
        this.showScene = function ( fnScene, sceneArgs )
        {
            ...
            // Call for a redraw, need if noLoop() is used
            this.parentObj.redraw();
        }
